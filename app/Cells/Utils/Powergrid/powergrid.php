<div class="w-full">
    ...
    <div class="flex flex-row justify-between mb-6">
        <div class="flex flex-row justify-center align-middle mx-2">
        ...
        <div class="flex flex-row justify-end align-middle">
            <button onclick="printTable()" <?= $emptyTable == true ? 'disabled' : '' ?> class="secondary-btn mx-2">
            ...
            <button id="download-xlsx" <?= $emptyTable == true ? 'disabled' : '' ?> onclick="fnExcelReport()" class="secondary-btn mx-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
    ...
    <div class="my-4 w-full flex flex-row">
        <?php if (isset($searchParamActive) && $searchParamActive) : ?>
            <form class="w-full flex flex-row mx-2 align-baseline" onsubmit="searchParam()" method="GET">
                <select id="columnSelect_<?= $tableId ?>" class="secondary-btn w-2/12 mx-2">
                    <?php foreach ($tableHeaders as $key => $value) : ?>
                        <option value='<?= $value ?>'><?= $value ?></option>
                    <?php endforeach; ?>
                </select>
                <input type="text" name="search" placeholder="Search" class="secondary-input h-full w-2/3">
                <div class="mx-2">
                    <button type="submit" class="secondary-btn h-full">
